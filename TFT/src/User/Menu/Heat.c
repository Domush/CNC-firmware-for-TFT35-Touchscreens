...
static HEATER heater = {{}, SPINDLE0, SPINDLE0};
static HEATER lastHeater = {{}, SPINDLE0, SPINDLE0};
...
/* Set current heater tool, spindle or hot bed */
void heatSetCurrentTool(TOOL tool) {
  if (tool >= HEATER_NUM) return;
  heater.tool = tool;
}
/* Get current tool, spindle or hot bed */
...
void heatSetCurrentToolSpindle(TOOL tool) {
  if (tool >= HEATER_NUM && tool < SPINDLE0) return;
  heater.spindle = tool;
  heater.tool = tool;
}
/* Get current spindle*/
TOOL heatGetCurrentToolSpindle(void) {
  return heater.spindle;
}
