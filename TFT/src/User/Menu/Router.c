...
void routerControl(u8 speed) {
  ...
  if (tmp.axis[X_AXIS] < 0 || tmp.axis[Y_AXIS] < 0) {  // *Don't allow power up while gantry is below X/Y zero
    popupReminder((u8*)"Cannot power up router while X/Y axis is negative.\n\nZero X/Y axis to enable.", (u8*)"Caution: Unsafe gantry position!");
    return;
  }
    ...
    switch (key_num) {
      ...
      case KEY_ICON_4:  // Router off
        if (isPrinting() && !isPause()) {
          popupReminder((u8*)"To avoid bit damage, stopping the router is not allowed during a print.\n\nPause the print to turn off the router", (u8*)"Not allowed");
        } else {
          routerControl(0);
        }
        break;
      ...
      case KEY_ICON_5:  // Swap bits
        if (isPrinting() && !isPause()) {
          popupReminder((u8*)"Cannot change bits while print in progress.\n\nPause print to enable bit change", (u8*)"Not allowed");
        } else {
          routerChangeBit();
        }
        break;
